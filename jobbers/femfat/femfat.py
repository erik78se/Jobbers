...
def process():
    ...
    questions = [
        ...
        inquirer.List("inputfiles", message="Choose inputfile", choices=inputfiles),
        inquirer.List(
            "template", message="Which template shall be used?", choices=template_files
        ),
        inquirer.Confirm(
            "starttime", message="Do you want to set a starttime?", default=False
        ),
    ]
    ...
    if answers.get("starttime"):
        ...
        if "lsf" in answers.get("template"):
            workloader = [
                inquirer.Text("start", message="[[year:][month:]day:]hour:minute?")
            ]
            workload_answers = inquirer.prompt(workloader)
